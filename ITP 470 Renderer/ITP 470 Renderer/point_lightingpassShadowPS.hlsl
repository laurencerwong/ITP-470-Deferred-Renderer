...
float4 main(PixelIn input) : SV_TARGET0
{
	...
	*/
	...
	float shadowMapSample = gShadowMap.Sample(linearTextureSampler, normalize(-pixToLight)).r + 0.003f;// *pointLight.mOuterRadius;
	//shadowMapSample += 100.1f;
	//shadowMapSample = (shadowMapSample == 0.0f) ? 10000.0f : shadowMapSample;
	//float measuredDepth = VecToDepth(pixToLight);
	float shadowFactor = (distToLight / pointLight.mOuterRadius - shadowMapSample < 0) ? 1.0f : 0.0f;//gShadowMap.SampleCmpLevelZero(ShadowMapSampler, -normalize(pixToLight), distToLight/pointLight.mOuterRadius).r;
	...
	output += outputDiffuse * diffuseFactor * diffuse *shadowFactor;
	...
	output += outputSpecular * specFactor * float4(specular.xyz, 1.0) * shadowFactor;
