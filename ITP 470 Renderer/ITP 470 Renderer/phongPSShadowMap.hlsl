...
//	float depth = 1.0f - input.pos.z / input.pos.w;
	...
	input.shadowTexCoord.xyz /= input.shadowTexCoord.w; //homogenize the vector
	...
	float depth = input.shadowTexCoord.z;
	...
	if (gDirLight.mColor.a > 0)
	{
		...
		if (NdotL > 0.0f)
		{
			...
			diffuse = gDirLight.mColor * NdotL *gMaterial.mDiffuse * shadowFactor;
			specular = saturate(gDirLight.mSpecularColor * dirSpecFactor * gMaterial.mSpecular * shadowFactor);
		}
	}
